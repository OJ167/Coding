...
i2c = I2C(0, sda=machine.Pin(SDA_PIN), scl=machine.Pin(SCL_PIN), freq=10000)
...
while True:
    if flag_value < flag_limit: #This sets the limit for how many ticks before new value is displayed
        ...
        RPM = (normalisedFlagLimit / normalisedDeltaTime) * 0.943 #Calculating RPM
        #seconds = deltaTime/1000000
        screen_update()
        #print(str(RPM))
        #print(str(normalisedDeltaTime))
        print(str(deltaTime))
        #print(str(seconds))
        #print("RPM = " + str(60/seconds))
        calctime = utime.ticks_us() - endTime
        print(str("calctime : " + str(calctime)))
