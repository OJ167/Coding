...
f2, ax2 = plt.subplots()
ax2.contourf(vort)
ax2.scatter(point_a[1],point_a[0],label = "point a") 
ax2.scatter(point_b[1],point_b[0],label = "point b")
ax2.scatter(point_c[1],point_c[0],label = "point c")
ax2.scatter(point_d[1],point_d[0],label = "point d")
plt.legend()
# plt.show()
...
f3, ax3 = plt.subplots()
ax3.quiver(u[0,:,:],v[0,:,:])
ax3.plot((point_a[1],point_a[0]), (point_b[1],point_b[0]), label = "Point a", linewidth=3) 
ax3.plot((point_b[1],point_b[0]), (point_c[1],point_c[0]), label = "Point b", linewidth=3)
ax3.plot((point_c[1],point_c[0]), (point_d[1],point_d[0]), label = "Point c", linewidth=3)
ax3.plot((point_d[1],point_d[0]), (point_a[1],point_a[0]), label = "Point d", linewidth=3)
plt.legend()
# plt.show()
...
#### line integral of tangent velocity #####
def path_integral_Tan_velocity(
        u, v
):
    Vel = np.sqrt(np.square(U)+np.square(V))
    ...
    sumpath1 = sum(abs(u[25, 25:75]))
    sumpath2 = sum(abs(v[25:75, 75]))
    sumpath3 = sum(abs(u[75, 75:25]))
    sumpath4 = sum(abs(v[75:25, 25]))

    Circulation = abs(sumpath1) + abs(sumpath2) - abs(sumpath3) - abs(sumpath4)

    return Circulation

Tan_circ = path_integral_Tan_velocity(U, V)
print("Tangent Velocity ", Tan_circ)

f3, ax3 = plt.subplots(sharex=True, sharey=True)
ax3.quiver(X, Y, U, (0.01*V), units='xy', pivot = 'middle', scale=scale)
ax3.contour((5*(Y**2))+(0.5*(X**2)))
plt.show()

path_tan = path_integral_Tan_velocity(U, V)
print("path tan synthetic: ", path_tan)
