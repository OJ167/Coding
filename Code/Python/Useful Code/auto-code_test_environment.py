...
port  = "COM5" # port that the Arduino is connected to - CHANGE THIS IF ARDUINO IS DISCONNECTED
...
Upiston = 200 # piston velocity in mm/s
Ring_Count = 10 # Number of rings to be generated
#####################################################################

#### Secondary tank principles ####
major_Delay = 5 # Delay between ring generations
minor_Delay = 5 #time after ring is fired for reload
RPM = 0.00 # Rotation rate used for saving data correctly
CameraFPS = 60 #camera framerate
frames = 1500 #number of frames to capture
########
...
Volume = Stroke/1000 * CSA #Fluid Volume per Injection in m^3
...
def file_conrtol(directoryNew): # function to automatically create the file structure for the remaining runs
    ...
    parameterDir = str("RPM-" + str(RPM) + "__Upiston-" + str(Upiston) + "__Stroke-" + str(Stroke) + "__FPS-" + str(CameraFPS))
    print(parameterDir)
    # Set root directory

    # Create directory within root with control parameters
    folder = os.path.join(directoryNew, parameterDir)
...
def camera_control(finalDir): # function to trigger the camera recording and save to the right location
    ...
    for n, img in enumerate(imgs):
        Image.fromarray(img).save(os.path.join(output_dir, '%08d.tiff' % n))
...
# time.sleep(Delay)
# command_converter()
# print(ring_control(arduino_Speed, arduino_Steps))
# print(piston_return(arduino_Speed, arduino_Steps))
# piston_return(arduino_Speed, arduino_Steps)
