...
data = np.array(read_excel("/Volumes/Untitled/17ishRPM.xlsx"))
...
t = np.linspace(0, 10, 250, endpoint=False)
plt.plot(t, signal.square(2 * np.pi * 5 * t))
plt.ylim(-2, 2)
plt.show()


squarefft, squarefreq = FFT(signal.square(2 * np.pi * 5 * t), 250) 
plt.plot(squarefreq, squarefft)
plt.show()

fftresult,fftfreq = FFT(X, 1000000/18171)
