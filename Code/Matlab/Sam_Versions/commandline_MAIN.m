...
directories = {uigetdir, uigetdir};%, uigetdir};%, uigetdir, uigetdir, uigetdir};%,uigetdir,uigetdir};                      % Comment out how many are not needed
...
for aa = 1:nRuns
    ...
    disp(directory)
    nr_of_cores = 8; % integer, 1 means single core, greater than 1 means parallel (42 on big machine)
    ...
    save(saveDirectory1,'u_filtered','v_filtered', 'vorticity', '-v7.3');
    saveDirectory2 = fullfile(parentFolder,'data', 'PIV_export_unfiltered.mat');
    save(saveDirectory2,'u','v', 'vorticity', '-v7.3');
    timer = toc;
    if pairwise == 0  
