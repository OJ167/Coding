...
const byte inkPin = 4; // pin for the ink pump control.
int interruptPin = 2; //pin for the microswitch using attachInterrupt();


// direction Digital 9 (CCW), pulses Digital 8 (CLK)
AccelStepper stepper(1, 13, 9);
...
void setup()
{
  ...
  attachInterrupt(digitalPinToInterrupt(interruptPin), stopMotor, LOW);
  ...
  //Ink Pump Pin
  pinMode(inkPin, OUTPUT);
  digitalWrite(inkPin, HIGH);
  ...
  stepper.setAcceleration(300000); //ACCELERATION = Steps /(second)^2
...
void checkSerial() //method for receiving the commands
{
  ...
  if (newData == true) //if we received something (see above)
  {
    ...
    {
      runallowed = false; //we still keep running disabj, since we just update a variable
    ...
    {
      Serial.println("INJECTING INK"); //print action
      digitalWrite(inkPin, LOW); //set the pump to run
      delay(2000); //Alter this value to change injection duration
      digitalWrite(inkPin, HIGH); //turn off the pump
...
void stopMotor()//function activated by the switch when pressed
{
